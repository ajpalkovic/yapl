!function($) {
   ...
   */
  function _parse(currentRule, startPos) {
   ...
   */
  function _parseRule(ruleName, rule, startPos) {
   ...
   */
  function _match(expectedTokenType) {
   ...
   * token.
   */
  function passesNegativeLookahead(negativeToken) {
    var nextToken = this.lexer.peek();
    return !nextToken || nextToken.type !== negativeToken;
  }

  /**
   * Error function for the parser that is called when the parse fails.
   */
  function error() {
  ...
  window.Parser = Class.create({
    ...
    parse: parse,
    _parse: _parse,
    _parseRule: _parseRule,
    _match: _match,
    passesNegativeLookahead: passesNegativeLookahead,
    error: error
  });
}(jQuery);
